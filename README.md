# Hey, I'm Jason

**Contributor** at [Osmosis Labs](https://osmosis.zone) from Cape Town, South Africa.

I also build tools and utilities for the Osmosis and Cosmos ecosystem:

## Pinned Projects

| Project | Description |
|---------|-------------|
| [**CCTP Relayer**](https://github.com/jasbanza/cctp-relayer) | CCTP relayer implementation |
| [**Cosmos Chain Wallet Checker**](https://github.com/jasbanza/cosmos-chain-wallet-checker) | Checks wallet balances at specific block heights |
| [**Osmosis Cancel Limitorders**](https://github.com/jasbanza/osmosis-cancel-limitorders) | Look up and cancel Osmosis limit orders |
| [**Osmosis Tick Price**](https://github.com/jasbanza/osmosis-tick-price) | Get the price of a pool's tick on Osmosis |
| [**Osmosis List Pools**](https://github.com/jasbanza/osmosis-list-pools) | Advanced Osmosis pool search |
| [**TFM Cancel Orders**](https://github.com/jasbanza/tfm-cancel-orders) | dApp to cancel orders on TFM |
| [**Autoswap Cancel Orders**](https://github.com/jasbanza/autoswap-cancel-orders) | Cancel missing limit orders on Autonomy's Autoswap (Osmosis) |
| [**Cosmos Node Status Report**](https://github.com/jasbanza/cosmos-node-status-report) | Tests cosmos-sdk blockchain nodes (RPC/REST/gRPC) |
| [**Osmosis IBC Monitor**](https://github.com/jasbanza/osmosis-ibc-monitor) | Monitors IBC client status for Osmosis assets |

Browse all my Osmosis & Cosmos utilities on the **[interactive dashboard](https://jasbanza.github.io/osmosis-utils/)**.

## Gists
| Title | Description |
|---------|-------------|
| [**Axelar GMP Rescue**](https://gist.github.com/jasbanza/64bf5a2405b1d6cf9931b58b07090e07) | Step by step guide to resume stuck Axelar GMP transfers|


## Links

[![Twitter](https://img.shields.io/badge/@jasbanza-000?style=flat&logo=x&logoColor=white)](https://twitter.com/jasbanza)
[![Linktree](https://img.shields.io/badge/Linktree-43E55E?style=flat&logo=linktree&logoColor=white)](https://linktr.ee/jasbanza)
